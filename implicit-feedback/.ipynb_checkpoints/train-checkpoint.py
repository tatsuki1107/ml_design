...
def train_ranker(
    ...
    assert estimator in ["naive", "ips", "ideal"], f"estimator must be 'naive', 'ips' or 'ideal', but {estimator} is given"
    if pow_used is None:
        pow_used = pow_true
    ...
    for _ in tqdm(range(n_epochs)):
        loader = DataLoader(
            train, batch_size=batch_size, shuffle=True, collate_fn=train.collate_fn()
        )
        score_fn.train()
        for batch in loader:
            if estimator == "naive":
