...
def train_ranker(
    ...
    """
    assert estimator in [
        "naive", "ips-via-rec", "ips-platform"
    ], f"estimator must be 'naive', 'ips-via-rec', or 'ips-platform', but {estimator} is given"
    assert objective in [
        "via-rec", "platform"
    ], f"objective must be 'via-rec', 'platform', but {objective} is given"
    
    ndcg_score_list = list()
    for _ in tqdm(range(n_epochs)):
        loader = DataLoader(
            train, batch_size=batch_size, suffle=True, collate_fn=train.collate_fn()
        )
        score_fn.train()
        for batch in loader:
