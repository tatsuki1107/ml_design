...
class MatrixFactorization:
    ...
    パラメータ
    ---------
    k:int                     ユーザ・アイテムベクトルの次元数.
    learning_rate: float      学習率.
    reg_param: float          正則化項のハイパーパラメータ.
    random_state: int         モデルパラメータの初期値を司る乱数.
    ...
    def fit(
        ...
        for _ in tqdm(range(n_epochs)):
            ...
            #テストデータにおける嗜好度合いの予測誤差を計算する
            r_hat_test = elf.predict(data=test)
    ...
    def _predict_pair(self, user: int, item: int) -> float:
        """与えられたユーザとアイテムのペア(u,i)の嗜好度合いを予測."""
        return self.P[user] @ self.Q[item]
    
    def predict(self, data: np.ndarray) -> np.ndarray:
        """与えられたデータセットに含まれる全ユーザ・アイテムペアの嗜好度合いを予測."""
